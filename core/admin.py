...
class SoftDeleteAdmin(admin.ModelAdmin):
    ...
    def is_deleted_display(self, obj):
        if obj.is_deleted:
            return format_html(
                '<span style="color: red; font-weight: bold;">Soft Deleted</span>'
            )
    ...
    def hard_delete_selected(self, request, queryset):
        ...
        self.message_user(
            request, f"{deleted_count} soft-deleted record(s) permanently deleted!"
        )

    hard_delete_selected.short_description = (
        "Permanently delete selected soft-deleted records"
    )
