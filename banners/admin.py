...
class BannerAdmin(HideBaseModelFieldsMixin, FormatBaseModelFieldsMixin, SoftDeleteAdmin, ModelAdmin):
    list_display = (
        'id', 'banner_image', 'title', 'subtitle', 'status', 'display_location', 'display_order', 
        'formatted_start_date', 'formatted_end_date', 'show_call_to_action', 'is_deleted_display'
    )
    search_fields = ('title', 'subtitle', 'display_location')
    list_filter = ('status', 'display_location', 'show_call_to_action', 'start_date', 'end_date')
    ...
    def formatted_start_date(self, obj):
        return obj.start_date.strftime('%Y-%m-%d %H:%M:%S') if obj.start_date else "Not Set"
